...
pd.to_pickle(mf_lr_clf,path+'mf_lr_desc_clf.pkl')

lsvc = LinearSVC(C=0.2,dual=True,random_state=seed)
mf_lsvc_clf = make_mf_lsvc(X ,y, lsvc, X_t,n_round=10)
pd.to_pickle(mf_lsvc_clf,path+'mf_lsvc_desc_clf.pkl')
...
# pd.to_pickle(mf_gblinear_clf,path+'mf_gblinear_desc_clf.pkl')
...
# rf = XGBLinearRegressor(learning_rate=0.025,max_depth=10,gamma=0.2,reg_alpha=0,reg_lambda=1,max_iter=1200,subsample=0.9,colsample_bytree=0.8,seed=seed,booster='gbtree')
# mf_gbtree_clf = make_mf_gblinear(X_emb ,y, gbtree, X_t_emb,n_round=10)
...
#     verbose=True,
# )
# mf_rf_clf = make_mf_lr(X_emb ,y, rf, X_t_emb,n_round=10)
